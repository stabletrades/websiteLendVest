...
(function($){
    ...
    function tglcls(tigger,action,connect,connect2){
        ...
        $toglr.on("click",function(){
            ...
            $body_m.toggleClass('user-sidebar-shown');
            if($tgcon.hasClass('active')){
                $tgcon.removeClass('active');
                $body_m.removeClass('user-sidebar-shown');
            }
    ...
    if($tranx_table.length > 0){
       var $tranx_table_fltr = $tranx_table.DataTable({
           "ordering": false,
           autoWidth: false,
           "dom":'<"row"<"col-10 text-start"f><"col-2 text-end"<"data-table-filter dropdown">>><"row"<"col-12"<"overflow-x-auto"t>>><"row"<"col-sm-6 text-start"p><"col-sm-6 text-sm-end"i>>',
           "pageLength": 7, 
    ...
    if($activity_table.length > 0){
        $activity_table.DataTable({
           ordering: false,
           autoWidth: false,
           dom:'<"row"<"col-12"<"overflow-x-auto"t>>><"row align-items-center"<"col-sm-6 text-start"p><"col-sm-6 text-sm-end text-center"<"clear-table">>>',
           pageLength: 7, 
           bPaginate : $('.data-table tbody tr').length>7,
           iDisplayLength: 7,
           language: {
    ...
    if($refferal_table.length > 0){
        $refferal_table.DataTable({
           ordering: false,
           autoWidth: false,
           dom:'<"row"<"col-12"<"overflow-x-auto"t>>><"row align-items-center"<"col-sm-6 text-start"p><"col-sm-6 text-sm-end text-center"i>>',
           pageLength: 5, 
    ...
    if($payment_check.length > 0){
        $payment_check.on('change', function(){
            ...
            $payment_btn.attr('data-bs-target', _thisHash)
        });
    }
